...
public class OPL extends Election{
    ...
    /**
     * ranks the candidates in thier respective parties based on their votes recieved
     * uses sortCandidates() helper function
     * @param partyCandidates the list of party candidate pairings in a HashMap
     * @param candidateVotes An arrayList of arraylists storing candidate names and the number of votes they recieved
     */
    ...
    /**
     * Sorts the candidates using a custom comparator to order them based on the number of votes
     * they recieved. Uses java.util.Comparator and java.util.Collections and getVotes helper function to do so
     * @param candidateVotes the list of candidates to be sorted by the helper function
     */
    ...
    private int getVotes(ArrayList<ArrayList<Object>> candidateVotes, String candidate){
        for (ArrayList<Object> entry : candidateVotes) { // loop through entrys to compare to candidate name
            if (entry.get(0).equals(candidate)) {
                return (int) entry.get(1); // return the first index of the inner Arraylist as its the number of votes for that candidate
            }
        }
        return -1; // if nothing is found then return a default value, -1 in this case since you can never have negative votes
    }
}
